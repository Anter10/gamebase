...
import { CopyMessageInterface, GameInitInterface, ImageAdInterface, OSAdinterface, RewardedAdInterface, SdkModuleInterface, ShareInterface, ToastInterface, WechatLoginInterface, ZhikeAdInterface } from "./SdkInterface";
...
export module SdkModule {
    ...
    /**@description 渲染静态图广告 */
    export function render_image_ad(image_ad_interface: ImageAdInterface) {
        if (SdkModule.isAndroid()) {
            sdk_module_interface.show_image_ad_success_callback = image_ad_interface.success;
            sdk_module_interface.show_image_ad_fail_callback = image_ad_interface.fail;
            jsb.reflection.callStaticMethod(`${android_class_name}`, "renderImageAd", `(III)V`, image_ad_interface.width, image_ad_interface.height, image_ad_interface.top);
        }
    }

    /**@description 显示静态图广告成功的回调 */
    export function image_ad_show_success(res: any) {
        if (sdk_module_interface.show_image_ad_success_callback) {
            sdk_module_interface.show_image_ad_success_callback(res);
        }
    }

    /**@description 显示静态图广告失败 */
    export function image_ad_show_fail(res: any) {
        if (sdk_module_interface.show_image_ad_fail_callback) {
            sdk_module_interface.show_image_ad_fail_callback(res);
        }
    }

    /**@description 关闭静态图广告 */
    export function close_image_ad() {
        if (SdkModule.isAndroid()) {
            jsb.reflection.callStaticMethod(`${android_class_name}`, "closeImageAd", `()V`);
        }
    }

    /**@description 调用toast */
    export function show_toast(toast_interface: ToastInterface) {
        if (SdkModule.isAndroid()) {
            jsb.reflection.callStaticMethod(`${android_class_name}`, "showToast", `(${S})V`, toast_interface.text);
        }
    }
}
