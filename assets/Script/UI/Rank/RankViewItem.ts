...
class RankViewItem extends BaseNode {
    ...
    update_data(rank_item_interface: RankItemInterface ,isSelf?: boolean){
        ...
        if (isSelf) { // 如果是自己，特殊处理
            ...
            Loader.load_texture(`./UI/Rank/Normal/rankUI/wathet/sprite_self_item_bg` ,(texture: cc.Texture2D) => {
                this.rank_item_sprite.spriteFrame = new cc.SpriteFrame(texture);
            });
            if(rank_item_interface.rank == 0){
                Loader.load_texture(`./UI/Rank/Normal/rankUI/wathet/sprite_item_norank` ,(texture: cc.Texture2D) => {
