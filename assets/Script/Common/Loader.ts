...
class Loader {
    ...
    /**@description 加载图集资源 */
    static load_sprite_atlas(path: string, call_back: Function) {
        cc.resources.load(path, cc.SpriteAtlas, (error: Error, atlas: cc.SpriteAtlas) => {
            if (!error) {
                cc.log(`加载图集${path}成功`);
                call_back && call_back(atlas);
            }
            else {
                cc.log(`图集资源${path}加载失败!`);
            }
        })
    }
